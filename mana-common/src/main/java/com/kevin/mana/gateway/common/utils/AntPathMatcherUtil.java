...
public final class AntPathMatcherUtil {
   ...
   * assuming that arbitrary permutations of patterns are coming in, with little chance for
  ...
  protected boolean doMatch(String pattern, String path, boolean fullMatch,
    ...
    if (path == null || path.startsWith(this.pathSeparator) != pattern
    ...
    if (pathIdxStart > pathIdxEnd) {
      ...
      if (pattIdxStart == pattIdxEnd && pattDirs[pattIdxStart].equals("*") && path
   ...
   * (see {@link #setCachePatterns}), creating a new AntPathStringMatcher instance if no cached copy
   * is found.
   * <p>When encountering too many patterns to cache at runtime (the threshold is 65536),
   * it turns the default cache off, assuming that arbitrary permutations of patterns are coming in,
   ...
   * <li>'{@code /docs/cvs/*.html}' and '{@code /docs/cvs/commit.html} -> '{@code
   * commit.html}'</li>
   * <li>'{@code /docs/**}' and '{@code /docs/cvs/commit} -> '{@code cvs/commit}'</li>
  ...
  public String extractPathWithinPattern(String pattern, String path) {
    String[] patternParts = tokenizeToStringArray(pattern, this.pathSeparator, this.trimTokens,
        true);
   ...
   * the first pattern contains a file extension match (e.g., {@code *.html}). In that case, the
   * second pattern will be merged into the first. Otherwise, an {@code IllegalArgumentException}
   * will be thrown.
   * <h3>Examples</h3>
   * <table border="1">
   ...
   * a list so that more specific patterns (without URI templates or wild cards) come before generic
  ...
  public String[] tokenizeToStringArray(String str, String delimiters, boolean trimTokens,
    ...
    return (CollectionUtils.isNotEmpty(tokens) ? tokens.toArray(EMPTY_STRING_ARRAY)
        : EMPTY_STRING_ARRAY);
  }
